...
class GroupingConcatOpHandlerTest(tf.test.TestCase):
  ...
  def testGetConcatOpAxis(self):
    img = tf.zeros([7, 12, 12, 3])
    self.assertEqual(
        concat_op_handler._get_concat_op_axis(tf.concat([img], 2), 2))
    self.assertEqual(
        concat_op_handler._get_concat_op_axis(tf.concat([img, img], 3), 3))
    self.assertEqual(
        concat_op_handler._get_concat_op_axis(tf.concat([img, img, img], 1), 1))
    # Concat one element edge case:
    self.assertEqual(
        concat_op_handler._get_concat_op_axis(tf.concat([img], 3), 3))
